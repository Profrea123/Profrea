                                                ...
                                                <div class="card">
                                                    ...
                                                    <h5 class="card-title">Condition</h5>
                                                        <p class="card-text">
                                                            ...
                                                            @if(isset($patient_condition) && $patient_condition !='')
                                                            <ol class="list-group list-group-numbered">
                                                                @foreach($patient_condition as $key => $c)
                                                                <li class="list-group-item"> {{ $c->condition }} <a class="float-right" href="{{ url('doctor/patient-condition-delete/'.$c->pc_id) }}"><i class="fas fa-trash text-danger"></i></a> </li>
                                                                @endforeach
                                                            </ol>
                                                            @else
                                                            {{ 'No Data Found' }}
                                                    ...
                                                    <a href="#" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#addCondition">Add New Condition</a>
                                                    </div>
                                                    ...
                                                    <div class="card-body text-muted">
                                                        <p class="card-text">
                                                            @if(isset($condition) && $condition !='')
                                                                <ol class="list-group list-group-numbered">
                                                                    ...
                                                                    <li class="list-group-item showpopup"> {{$c->title }} <a class="float-right" href="javascript:void(0)" data-value="{{ $c->title }}"><i class="fas fa-plus-circle text-success"></i></a> </li>
                                                                    @endforeach
                                                                </ol>
                                                                @else
                                                                {{ 'No Data Found' }}
                                                        ...
                                                        @if(isset($redirectDetails->complete_tab) && $redirectDetails->complete_tab >=6)
                                                            <a href="{{ route('doctor.view-patient-medicine') }}" class="btn btn-success">Next</a>
                                                        @endif
...
<div class="modal fade" id="addCondition" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="staticBackdropLabel">Add Condition</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form action="{{ route('doctor.casesheet.add-patient-condition') }}" method="post">
            @csrf
            <div class="form-group">
                <label for="healthCondition">Health Condition</label>
                <input type="text" name="condition" id="healthCondition" class="form-control" value="">
            </div>
            <!-- <div class="row">
            ...
            <input type="hidden" name="id" value="{{ isset($patient_condition->pc_id)?$patient_condition->pc_id:'' }}">
            ...
            <button type="submit" name="conditionSubmit" value="true" class="btn btn-primary mt-2">Save</button>
        </form>
      </div>
    </div>
  </div>
