                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header">
                            <h3 class="card-title mt-2">List of Request Refund</h3>
                        ...
                        <div class="card-body">
                            ...
                            <div class="row">
                                ...
                                @if (!empty(request()->booking_no) || !empty(request()->date_time) || !empty(request()->doctor) || !empty(request()->user) || !empty(request()->type))
                                  <a href="{{route('admin.booking_cancel.indexCancel')}}"><button type="button" class="ml-2 btn btn-rounded btn-danger filter"><i class="fa fa-redo"></i></button> </a>
                                @endif
                            </div>
                            <div class="table-responsive">
                                <table class="table table-bordered ">
                                    ...
                                    @if(count($booking_cancel) > 0)
                                    <tbody>
                                        @foreach ($booking_cancel as $booking)
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <a class="btn btn-info" href="{{ route('admin.booking_cancel.showCancel',$booking->booking_no) }}">Show</a>
                                            </td>
                                        </tr>
                                        @endforeach
                                    </tbody>
                            ...
                            {{$booking_cancel->links('pagination::bootstrap-4')}}
                        </div>
  ...
  <script>
    $(document).ready(function(){
      $('.filter').click(function (){
        ...
        var user = $('#user').val();
        var type = $('#type').val();
        var url = "{{route('admin.booking_cancel.indexCancel')}}"+'?booking_no='+booking_no+'&date_time='+date_time+'&doctor='+doctor+'&user='+user+'&type='+type;
